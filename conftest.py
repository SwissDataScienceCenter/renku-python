...
def dataset_responses():
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        rsps.add_callback(
            responses.GET, "http://example.com/file", callback=request_callback
        )
        rsps.add_callback(
            responses.GET, "https://example.com/file", callback=request_callback
        )
        rsps.add_callback(
            responses.GET,
...
def old_bare_repository(request, tmpdir_factory):
    ...
    compressed_repo_path = (
        Path(__file__).parent
        / "tests"
        / "fixtures"
        / "{0}.tar.gz".format(request.param["name"])
    )
...
def old_repository(tmpdir_factory, old_bare_repository):
    ...
    yield {
        "repo": Repo(
            old_bare_repository["path"].strpath, search_parent_directories=True
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        def version_callback(request):
            return (
                ...
                {"Content-Type": "application/json"},
                json.dumps(
                    {"status": "OK", "data": {"version": "4.1.3", "build": "abcdefg"}}
                ),
            )
