...
function getTemplates(baseUrl, templatesRef) {
  const res = http.get(
    `${baseUrl}/ui-server/api/renku/templates.read_manifest?url=https%3A%2F%2Fgithub.com%2FSwissDataScienceCenter%2Frenku-project-template&ref=${templatesRef}`
  );
  if (
    !check(res, {
      "reading templates succeeded with 2XX": (res) =>
        res.status >= 200 && res.status < 300,
      "reading templates response has no error": (res) =>
        res.json().error === undefined,
      "reading templates response more than zero templates": (res) =>
        res.json().result.templates.length > 0,
    })
  ) {
    fail(
      `reading templates failed with status code ${res.status} and response ${res.body}`
    );
  }
  return res;
}
