...
def test_login_no_endpoint(runner, project, mock_login, args):
...
@pytest.mark.parametrize("args", [[], ["--git"]])
def test_login_invalid_endpoint(runner, project, mock_login, args):
...
def test_login_to_multiple_endpoints(runner, project, mock_login, with_injection):
...
def test_login_non_git(runner, project, directory_tree):
