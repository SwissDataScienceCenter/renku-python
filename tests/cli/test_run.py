...
@pytest.mark.shelled
def test_run_untracked(runner, project, run_shell):
    """Test tracking of run command in dirty repo with untracked files."""
    # Run a shell command with pipe.
    output = run_shell('echo "aa" > datafile', sleep_for=1)
    # Assert expected empty stdout.
    assert output[0] == b''
    # Assert not allocated stderr.
    assert output[1] is None
    ...
    # Check that datafile is not committed.
    repo = git.Repo(project)
    assert 'datafile' in repo.untracked_files


@pytest.mark.shelled
def test_run_staged(runner, project, run_shell):
    """Test tracking of run command n dirty repo with staged files."""
    # Run a shell command with pipe.
    output = run_shell('echo "aa" > datafile && git add datafile', sleep_for=1)
    ...
    # Assert datafile is still on stage (and not committed)
    assert b'datafile' in output[0]
    # Assert not allocated stderr.
    assert output[1] is None

    repo = git.Repo(project)
    diff = {item.a_path for item in repo.index.diff('HEAD')}
    assert 'datafile' in diff
