...
def test_dataset_import_real_doi(runner, project, doi, sleep_after):
    ...
    result = runner.invoke(cli, ["dataset", "import", doi["doi"]], input=doi["input"])
...
@pytest.mark.parametrize(
    "doi", [("10.5281/zenodo.5979642342", "Zenodo"), ("10.7910/DVN/S8MSVFXXXX", "DVN")]
)
@pytest.mark.integration
def test_dataset_import_fake_doi(runner, project, doi):
    ...
    assert 2 == result.exit_code
    assert (
        "URI not found." in result.output
...
@pytest.mark.integration
def test_dataset_export_upload_file(runner, project, tmpdir, client, zenodo_sandbox):
...
@pytest.mark.integration
def test_dataset_export_upload_tag(runner, project, tmpdir, client, zenodo_sandbox):
    """Test successful uploading of a file to Zenodo deposit."""
    result = runner.invoke(cli, ["dataset", "create", "my-dataset"])
    assert 0 == result.exit_code
    assert "OK" in result.output
...
@pytest.mark.integration
def test_dataset_export_published_url(runner, project, tmpdir, client, zenodo_sandbox):
...
@pytest.mark.integration
def test_export_dataset_wrong_provider(runner, project, tmpdir, client, zenodo_sandbox):
    ...
    result = runner.invoke(cli, ["dataset", "export", "my-dataset", "notzenodo"])
    assert 2 == result.exit_code
...
@pytest.mark.integration
def test_export_dataset_unauthorized(runner, project, client, tmpdir, zenodo_sandbox):
...
def test_dataset_update_remove_file(client, runner):
    ...
    result = runner.invoke(
        cli,
        ["dataset", "update", "--ref", "v0.5.0", "--delete"],
        catch_exceptions=False,
    )
    assert 0 == result.exit_code
...
@pytest.mark.integration
@pytest.mark.parametrize(
    "params", [[], ["-I", "CHANGES.rst"], ["-I", "CH*"], ["dataset-1", "dataset-2"]]
)
...
def test_import_from_renku_project(tmpdir, client, runner):
    ...
    remote = read_dataset_file_metadata(
        remote_client, "zhbikes", "2019_verkehrszaehlungen_werte_fussgaenger_velo.csv"
    )
...
@pytest.mark.integration
@pytest.mark.parametrize(
    "path,expected_path", [("", "datasets-test"), (".", "."), ("new-name", "new-name")]
)
