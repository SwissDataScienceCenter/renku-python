...
def test_run_succeeds_normally(renku_cli, client, subdirectory):
    ...
    foo = os.path.relpath(config.path / "foo", os.getcwd())
...
def test_explicit_outputs_and_normal_outputs(renku_cli, client, subdirectory):
    ...
    foo = os.path.relpath(config.path / "foo", os.getcwd())
    os.mkdir(foo)
    bar = os.path.relpath(config.path / "bar", os.getcwd())
    renku_cli("run", "touch", bar)
    baz = os.path.relpath(config.path / "baz", os.getcwd())
...
def test_output_directory_with_output_option(runner, renku_cli, client, subdirectory):
    ...
    outdir = os.path.relpath(config.path / "outdir", os.getcwd())
    ...
    assert (config.path / "outdir" / "foo").exists()
    assert (config.path / "outdir" / "bar").exists()
...
def test_explicit_inputs_and_outputs_are_listed(renku_cli, client):
    ...
    foo = Path(os.path.relpath(config.path / "foo", os.getcwd()))
    foo.mkdir()
...
def test_explicit_inputs_in_subdirectories(client, runner):
    ...
    write_and_commit_file(client.repository, config.path / "foo" / "bar", "new changes")
    ...
    assert 0 == result.exit_code, format_result_exception(result)
    assert (config.path / "foo" / "bar").exists()
    assert (config.path / "script.sh").exists()
    assert (config.path / "output").exists()
