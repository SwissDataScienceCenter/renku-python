...
from renku.core.commands.providers import (
    DataverseProvider,
    ProviderFactory,
    ZenodoProvider,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("semi valid-name", "semi_validname"),
        ("dataset/new", "datasetnew"),
        ("/dataset", "dataset"),
        ("dataset/", "dataset"),
    ],
)
...
def test_add_and_create_dataset(directory_tree, runner, project, client):
    ...
    result = runner.invoke(
        cli,
        ["dataset", "add", "new-dataset", str(directory_tree)],
    ...
    result = runner.invoke(
        cli,
        ["dataset", "add", "--create", "new-dataset", str(directory_tree)],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
...
def test_repository_file_to_dataset(runner, project, client):
    ...
    # add data
    result = runner.invoke(
        cli, ["dataset", "add", "dataset", "a"], catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_relative_import_to_dataset(tmpdir, runner, client):
    ...
    assert os.stat(os.path.join("data", "dataset", "first", "second", "second.txt"))
...
def test_usage_error_in_add_from_url(runner, client, params, message):
    ...
    result = runner.invoke(
        cli, ["dataset", "add", "remote", "--create"] + params, catch_exceptions=False
    )
...
def test_add_from_local_repo_warning(runner, client, data_repository, directory_tree):
    ...
    result = runner.invoke(
        cli,
        ["dataset", "add", "dataset", "--create", str(directory_tree)],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
...
def test_dataset_add_with_link(tmpdir, runner, project, client):
    ...
    # add data
    result = runner.invoke(
        cli, ["dataset", "add", "my-dataset", "--link"] + paths, catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_datasets_ls_files_tabular_dataset_filter(tmpdir, runner, project):
    ...
    # add data to dataset
    result = runner.invoke(
        cli, ["dataset", "add", "my-dataset"] + paths, catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_datasets_ls_files_tabular_patterns(tmpdir, runner, project):
    ...
    result = runner.invoke(
        cli, ["dataset", "ls-files", "--include=**/file*", "--exclude=**/file_2"]
    )
    assert 0 == result.exit_code
...
def test_dataset_unlink_file(tmpdir, runner, client):
    ...
    result = runner.invoke(
        cli, ["dataset", "unlink", "my-dataset", "--include", new_file.basename, "-y"]
    )
    assert 0 == result.exit_code
...
def test_dataset_edit(runner, client, project):
    ...
    result = runner.invoke(
        cli, ["dataset", "edit", dataset.identifier], input="wq", catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_dataset_edit_dirty(runner, client, project):
    ...
    result = runner.invoke(cli, ["dataset", "edit", dataset.identifier], input="wq")
    assert 0 == result.exit_code
...
@pytest.mark.parametrize(
    "uri",
    [
        "10.5281/zenodo.3363060",
        "doi:10.5281/zenodo.3363060",
...
def test_dataset_tag(tmpdir, runner, project):
    ...
    # add data to dataset
    result = runner.invoke(
        cli, ["dataset", "add", "my-dataset", str(new_file)], catch_exceptions=False
    )
    assert 0 == result.exit_code

    # tag dataset
    result = runner.invoke(
        cli, ["dataset", "tag", "my-dataset", "1.0"], catch_exceptions=False
    )
    assert 0 == result.exit_code
    ...
    result = runner.invoke(
        cli, ["dataset", "tag", "my-dataset", "aBc9.34-11_55.t"], catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_dataset_rm_tag(tmpdir, runner, project, client):
    ...
    result = runner.invoke(
        cli, ["dataset", "ls-tags", "my-dataset"], catch_exceptions=False
    )
    assert 0 == result.exit_code
    assert "1.0" in result.output
    assert "first tag!" in result.output
    ...
    result = runner.invoke(
        cli, ["dataset", "rm-tags", "my-dataset", "2.0"], catch_exceptions=False
    )
    assert 2 == result.exit_code
    assert "not found" in result.output

    result = runner.invoke(
        cli, ["dataset", "rm-tags", "my-dataset", "1.0"], catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_dataset_rm_tags_multiple(tmpdir, runner, project, client):
    ...
    for i in range(1, 4):
        # tag dataset
        result = runner.invoke(
            cli, ["dataset", "tag", "my-dataset", str(i)], catch_exceptions=False
        )
        assert 0 == result.exit_code

    result = runner.invoke(
        cli, ["dataset", "rm-tags", "my-dataset", "1", "2", "3"], catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_dataset_rm_tags_failure(tmpdir, runner, project, client):
    result = runner.invoke(
        cli, ["dataset", "rm-tags", "my-dataset", "1"], catch_exceptions=False
    )
