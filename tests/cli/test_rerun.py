...
@pytest.mark.shelled
def test_rerun_dirty_untracked(runner, project, run_shell):
    """Test rerun command on dirty repository with untracked files."""
    cwd = Path(project)
    data = cwd / 'data'
    data.mkdir()
    ...
    result = run_shell(
        'renku run cat data/step1.out > data/step2.out',
        sleep_for=1,
    )
    assert b'' == result[0]
    assert result[1] is None
    ...
    result = run_shell('renku rerun data/step2.out', sleep_for=1)
    assert 'success' in str(result[0])
    assert result[1] is None
