...
def test_update(runner, repository, renku_cli, client_database_injection_manager, provider, skip_metadata_update):
    """Test output is updated when source changes."""
    source = os.path.join(repository.path, "source.txt")
    output = os.path.join(repository.path, "output.txt")
    ...
    write_and_commit_file(repository, source, "changed content")
...
def test_update_multiple_steps(
    runner, repository, renku_cli, client_database_injection_manager, provider, skip_metadata_update
):
    """Test update in a multi-step workflow."""
    source = os.path.join(repository.path, "source.txt")
    intermediate = os.path.join(repository.path, "intermediate.txt")
    output = os.path.join(repository.path, "output.txt")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_multiple_steps_with_path(runner, repository, renku_cli, provider):
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_with_directory_paths(repository, renku_cli, provider):
    """Test update when a directory path is specified."""
    data = os.path.join(repository.path, "data", "dataset", "my-data")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_multiple_updates(runner, repository, renku_cli, provider):
    ...
    write_and_commit_file(repository, source, "more changed content")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_workflow_without_outputs(runner, repository, run, provider):
    ...
    write_and_commit_file(repository, source, "changes")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_siblings(repository, run, no_lfs_warning, provider):
    """Test all generations of an activity are updated together."""
    parent = os.path.join(repository.path, "parent.txt")
    brother = os.path.join(repository.path, "brother.txt")
    sister = os.path.join(repository.path, "sister.txt")
    siblings = [Path(brother), Path(sister)]

    write_and_commit_file(repository, parent, "content")
    ...
    write_and_commit_file(repository, parent, "changed content")
    ...
    repository.remove(brother)
    repository.commit("Brother removed")
    assert not os.path.exists(brother)

    write_and_commit_file(repository, parent, "more content")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_siblings_in_output_directory(repository, run, provider):
    """Files in output directory are linked or removed after update."""
    source = os.path.join(repository.path, "source.txt")
    output = Path(os.path.join(repository.path, "output"))  # a directory
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_relative_path_for_directory_input(repository, run, renku_cli, provider):
    """Test having a directory input generates relative path in CWL."""
    write_and_commit_file(repository, repository.path / DATA_DIR / "file1", "file1")

    assert 0 == run(args=["run", "ls", DATA_DIR], stdout="ls.data")

    write_and_commit_file(repository, repository.path / DATA_DIR / "file2", "file2")
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_no_args(runner, repository, no_lfs_warning, provider):
    ...
    before_commit = repository.head.commit
    ...
    assert before_commit == repository.head.commit


@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_with_no_execution(repository, runner, provider):
...
def test_update_overridden_output(repository, renku_cli, runner):
    ...
    write_and_commit_file(repository, a, "new content")
...
def test_update_overridden_outputs_partially(repository, renku_cli, runner):
...
def test_update_multiple_paths_common_output(repository, renku_cli, runner):
...
@pytest.mark.parametrize("provider", available_workflow_providers())
def test_update_with_execute(runner, repository, renku_cli, provider):
    """Test output is updated when source changes."""
    ...
    write_and_commit_file(repository, source1, "content_a")
    write_and_commit_file(repository, source2, "content_b")
    write_and_commit_file(repository, script, "cp $1 $2")
    ...
    assert "content_a" == (repository.path / output1).read_text()
    assert "content_b" == (repository.path / output2).read_text()
    ...
    assert "content_a_modified\n" == (repository.path / output1).read_text()
    assert "content_b_modified\n" == (repository.path / output2).read_text()
...
def test_update_with_external_files(runner, repository, directory_tree):
    ...
    path = repository.path / "data" / "my-dataset" / "directory_tree" / "file1"
    ...
    assert "updated file1" not in (repository.path / "output").read_text()
    ...
    assert 0 == result.exit_code, format_result_exception(result)
    assert "updated file1" in (repository.path / "output").read_text()


def test_update_ignore_deleted_files(runner, repository):
    """Test update can ignore deleted files."""
    deleted = repository.path / "deleted"
    write_and_commit_file(repository, "source", "source content")
    ...
    write_and_commit_file(repository, "source", "changes")
    delete_and_commit_file(repository, deleted)
...
def test_update_ignore_deleted_files_config(runner, repository):
...
def test_update_deleted_files_reported_with_siblings(runner, repository):
...
def test_update_deleted_files_reported_with_downstream(runner, repository):
