...
def test_update(runner, client, renku_cli, client_database_injection_manager, provider, skip_metadata_update):
    ...
    source = os.path.join(config.path, "source.txt")
    output = os.path.join(config.path, "output.txt")
...
def test_update_multiple_steps(
    ...
    source = os.path.join(config.path, "source.txt")
    intermediate = os.path.join(config.path, "intermediate.txt")
    output = os.path.join(config.path, "output.txt")
...
def test_update_relative_path_for_directory_input(client, run, renku_cli, provider):
    ...
    write_and_commit_file(client.repository, config.path / DATA_DIR / "file1", "file1")

    assert 0 == run(args=["run", "ls", DATA_DIR], stdout="ls.data")

    write_and_commit_file(client.repository, config.path / DATA_DIR / "file2", "file2")
...
def test_update_with_execute(runner, client, renku_cli, client_database_injection_manager, provider):
    ...
    assert "content_a" == (config.path / output1).read_text()
    assert "content_b" == (config.path / output2).read_text()
    ...
    assert "content_amodified\n" == (config.path / output1).read_text()
    assert "content_bmodified\n" == (config.path / output2).read_text()
...
def test_update_with_external_files(runner, client, directory_tree):
    ...
    assert "updated file1" not in (config.path / "output").read_text()
