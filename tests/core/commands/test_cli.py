...
def test_show_inputs(tmpdir_factory, project, runner, run):
    ...
    assert {str(imported_woop.resolve().relative_to(Path(project).resolve()))} == set(
...
def test_status_with_submodules(isolated_runner, monkeypatch):
    ...
    os.chdir("../bar")
    result = runner.invoke(
        cli, ["-S", "dataset", "add", "b", "../foo/data/f/woop"], catch_exceptions=False
    )
    assert 0 == result.exit_code
...
def test_status_consistency(runner, client, project):
    ...
    assert base_result.output.replace("somedirectory/", "") == comp_result.output
...
def test_input_directory(runner, project, run):
    ...
    result = runner.invoke(cli, ["show", "inputs"])
    assert set(
        str(p.relative_to(cwd)) for p in inputs.rglob("*") if p.name != ".gitkeep"
