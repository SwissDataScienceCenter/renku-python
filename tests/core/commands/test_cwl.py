...
def test_03_input(client, client_database_injection_manager):
    ...
    whale = Path(config.path) / "whale.txt"
    whale.touch()
    ...
    with client_database_injection_manager(client):
        plan = PlanFactory(argv, directory=config.path, working_dir=config.path).to_plan()
...
def test_base_command_detection(client, client_database_injection_manager):
    ...
    hello = Path(config.path) / "hello.tar"
    hello.touch()
...
def test_04_output(client, client_database_injection_manager):
    ...
    argv = ["tar", "xf", "hello.tar"]
    factory = PlanFactory(argv, directory=config.path, working_dir=config.path)

    # simulate run

    output = Path(config.path) / "hello.txt"
    output.touch()
...
def test_05_stdout(client, client_database_injection_manager):
    """Test stdout mapping."""
    output = Path(config.path) / "output.txt"
    output.touch()
    ...
    factory = PlanFactory(argv, directory=config.path, working_dir=config.path, stdout="output.txt")
...
def test_stdout_with_conflicting_arg(client, client_database_injection_manager):
    ...
    output = Path(config.path) / "lalala"
    output.touch()
    ...
    argv = ["echo", "lalala"]
    factory = PlanFactory(argv, directory=config.path, working_dir=config.path, stdout="lalala")
...
def test_stdin_and_stdout(argv, client, client_database_injection_manager):
    """Test stdout mapping."""
    input_ = Path(config.path) / "input.txt"
    input_.touch()
    output = Path(config.path) / "output.txt"
    output.touch()
    error = Path(config.path) / "error.txt"
    error.touch()
...
def test_existing_output_directory(client, runner, project, client_database_injection_manager):
    ...
    cwl, _ = CWLConverter.convert(plan, config.path)
