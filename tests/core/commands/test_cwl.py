...
def test_03_input(instance_path):
    ...
    argv = ["echo", "-f", "-i42", "--example-string", "hello", "--file=whale.txt"]
...
def test_05_stdout(instance_path):
    ...
    factory = CommandLineToolFactory(
        argv, directory=instance_path, working_dir=instance_path, stdout="output.txt"
    )
...
def test_stdout_with_conflicting_arg(instance_path):
    ...
    argv = ["echo", "lalala"]
    factory = CommandLineToolFactory(
        argv, directory=instance_path, working_dir=instance_path, stdout="lalala"
    )
...
def test_06_params(instance_path):
    ...
    assert parameters[0][0].type == "File"
    assert parameters[0][0].outputBinding.glob == "$(inputs.{0})".format(goodbye_id)
