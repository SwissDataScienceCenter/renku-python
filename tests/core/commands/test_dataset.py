...
def test_data_add(
    ...
    with raises(error):
        ...
        with client.with_dataset("dataset", create=True) as d:
            d.creator = [
                {"name": "me", "email": "me@example.com", "identifier": "me_id"}
            ]
...
def test_git_repo_import(client, dataset, tmpdir, data_repository):
    ...
    client.add_data_to_dataset(
        dataset, [os.path.join(os.path.dirname(data_repository.git_dir), "dir2")]
    )
...
def test_dataset_serialization(dataset):
    ...
    assert dataset.creator[0].email == dataset_metadata.get("creator")[0].get("email")
