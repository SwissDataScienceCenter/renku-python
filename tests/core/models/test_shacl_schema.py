...
def test_dataset_shacl(tmpdir, runner, project, client):
    ...
    force_dataset_path = (
        Path(__file__).parent.parent.parent / "fixtures" / "force_dataset_shacl.json"
    )

    force_datasetfile_path = (
        Path(__file__).parent.parent.parent
        / "fixtures"
        / "force_datasetfile_shacl.json"
    )

    force_datasettag_path = (
        Path(__file__).parent.parent.parent / "fixtures" / "force_datasettag_shacl.json"
    )
    ...
    with client.with_dataset("dataset") as dataset:
        ...
        rdf = pyld.jsonld.to_rdf(
            g, options={"format": "application/n-quads", "produceGeneralizedRdf": True}
        )
...
def test_project_shacl(project, client):
    ...
    g = project.asjsonld()
    rdf = pyld.jsonld.to_rdf(
        g, options={"format": "application/n-quads", "produceGeneralizedRdf": False}
    )
