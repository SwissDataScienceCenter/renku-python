...
def test_indirect_inputs_outputs(client):
    ...
    input_content = get_indirect_inputs_path(config.path).read_text()
    output_content = get_indirect_outputs_path(config.path).read_text()
...
def test_open_inputs(client):
    ...
    assert "some data" == (config.path / "input.txt").read_text()
...
def test_open_outputs(client):
    ...
    assert "some data" == (config.path / "output.txt").read_text()
...
def test_parameters(client):
    ...
    data = read_indirect_parameters(config.path)
