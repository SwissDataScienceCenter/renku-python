...
def _format_default(client, value):
    ...
    if isinstance(value, File):
        return os.path.relpath(str((client.workflow_path / value.path).resolve()))
    return value
...
def show_inputs(client, workflow):
    ...
    for input_ in workflow.inputs:
        click.echo(
            "{id}: {default}".format(
                id=input_.id, default=_format_default(client, input_.default)
            )
        )
    sys.exit(0)
...
def edit_inputs(client, workflow):
    ...
    types = {"int": int, "string": str, "File": lambda x: File(path=Path(x).resolve())}
    for input_ in workflow.inputs:
...
def rerun(client, revision, roots, siblings, inputs, paths):
    ...
    workflow = inputs(
        client,
        graph.ascwl(input_paths=roots, output_paths=output_paths, outputs=outputs),
    )
