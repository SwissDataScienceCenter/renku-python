...
@functools.lru_cache()
def storage_installed_locally(repository):
    """Verify that git-lfs is installed for the project."""
    repo_config = repository.get_configuration(scope="local")
    return repo_config.has_section('filter "lfs"')


@inject.autoparams()
def check_external_storage(client_dispatcher: IClientDispatcher):
    """Check if repository has external storage enabled.

    Arguments:
        client_dispatcher: The injected client dispatcher.
    ...
    is_storage_installed = storage_installed_locally(repository) and storage_installed()
    if project_properties.external_storage_requested and not is_storage_installed:
        raise errors.ExternalStorageDisabled(repository)

    if storage_installed_locally(repository) and not storage_installed():
        raise errors.ExternalStorageNotInstalled(repository)
...
def minimum_lfs_file_size(client):
    """The minimum size of a file in bytes to be added to lfs."""
    size = client.get_value("renku", "lfs_threshold")
...
def track_paths_in_storage(client, *paths):
    ...
    for path in paths:
        ...
        if (
            ...
            if file_size >= minimum_lfs_file_size(client):
...
@check_external_storage_wrapper
def list_unpushed_lfs_paths(client):
...
@check_external_storage_wrapper
def pull_paths_from_storage(client, *paths):
    """Pull paths from LFS."""
    client_dict = defaultdict(list)

    for path in expand_directories(paths):
        _, _, path = client.get_in_submodules(client.repository.head.commit, path)
        try:
...
@check_external_storage_wrapper
def clean_storage_cache(client, *paths):
    ...
    for path in expand_directories(paths):
        current_client, _, path = client.get_in_submodules(client.repository.head.commit, path)
        try:
            absolute_path = Path(path).resolve()
            relative_path = absolute_path.relative_to(project_properties.path)
        except ValueError:  # An external file
        ...
        if project_properties.path not in unpushed_paths:
            u_paths = list_unpushed_lfs_paths(current_client)
        ...
        if absolute_path in unpushed_paths[project_properties.path]:
            ...
            clients[project_properties.path] = current_client

    for client_path, paths in client_dict.items():
        current_client = clients[client_path]
        ...
        for path in paths:
            ...
            # get lfs sha hash
            old_pointer = current_client.repository.get_raw_content(path=path, revision="HEAD")
        ...
        current_client.repository.add(*paths)
...
def check_requires_tracking(client, *paths):
    ...
    for path in paths:
        ...
        if not absolute_path.is_dir():
            if renku_lfs_ignore().match_file(path):
                continue
            if os.path.getsize(absolute_path) < minimum_lfs_file_size(client):
                continue
...
def check_lfs_migrate_info(client, everything=False, use_size_filter=True):
    """Return list of file groups in history should be in LFS."""
    ref = ["--everything"] if everything else ["--include-ref", client.repository.active_branch.name]
    ...
    if use_size_filter:
        above = ["--above", str(minimum_lfs_file_size(client))]
        command += above
    ...
    if groups and use_size_filter:
        ...
        return check_lfs_migrate_info(client, everything=everything, use_size_filter=False)

    return groups


def migrate_files_to_lfs(client, paths):
    ...
    for old_commit_sha, new_commit_sha in commit_sha_mapping:
        old_commit = client.repository.get_commit(old_commit_sha)
        new_commit = client.repository.get_commit(new_commit_sha)
        processed = set()
