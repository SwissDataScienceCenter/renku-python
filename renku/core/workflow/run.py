...
def get_status(
    ...
    def mark_generations_as_stale(activity):
        for generation in activity.generations:
            generation_path = get_relative_path_to_cwd(config.path / generation.entity.path)
    ...
    paths = paths or []
    paths = get_relative_paths(base=config.path, paths=[Path.cwd() / p for p in paths])  # type: ignore
    ...
    for start_activity, entity in modified:
        usage_path = get_relative_path_to_cwd(config.path / entity.path)
    ...
    deleted_paths = {get_relative_path_to_cwd(config.path / d) for d in deleted_paths if not paths or d in paths}
