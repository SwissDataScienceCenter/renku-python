...
_RE_PATHNAME = (
    r"(?P<pathname>" r"(((?P<owner>[\w\-\.]+)/)?(?P<name>[\w\-\.]+)(\.git)?)?)"
)
...
_REPOSITORY_URLS = (
    ...
    _build(
        _RE_PROTOCOL, _RE_USERNAME_PASSWORD, _RE_HOSTNAME, r"/", _RE_PREFIXED_PATHNAME
    ),
    # https://user:pass@example.com:1234/owner/repo.git
    _build(
        _RE_PROTOCOL, _RE_USERNAME_PASSWORD, _RE_HOSTNAME, _RE_PORT, r"/", _RE_PATHNAME
    ),
...
class Range:
    ...
    def rev_parse(cls, git, revision):
        ...
        return cls(
            start=git.rev_parse(start) if start else None, stop=git.rev_parse(stop)
        )
