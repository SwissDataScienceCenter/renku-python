...
class CommandLineToolFactory(object):
    ...
    explicit_inputs = attr.ib(
        default=[], converter=lambda paths: [Path(path).resolve() for path in paths]
    )
            ...
            if stream and self.file_candidate(self.working_dir / stream):
                self.outputs.append(
                    CommandOutputParameter(
                        id="output_{0}".format(stream_name), type=stream_name
                    )
                )
    ...
    def watch(self, client, no_output=False):
        ...
        existing_directories = {
            str(p.relative_to(client.path)) for p in client.path.glob("**/")
        }
        ...
        if repo:
            ...
            candidates |= {
                o.a_path for o in repo.index.diff(None) if not o.deleted_file
            }
            ...
            for stream_name in ("stdout", "stderr"):
                ...
                if (
                    stream
                    and stream not in candidates
                    and Path(stream).resolve() not in self.explicit_outputs
                ):
    ...
    def split_command_and_args(self):
        ...
        while (
            args
            and re.match(self._RE_SUBCOMMAND, args[0])
    ...
    def guess_inputs(self, *arguments):
        ...
        output_streams = {
            getattr(self, stream_name) for stream_name in ("stdout", "stderr")
        }
        ...
        for index, argument in enumerate(arguments):
            ...
            if prefix:
                if argument.startswith("-"):
                    position += 1
                    yield CommandLineBinding(position=position, valueFrom=prefix)
                    prefix = None
            ...
            if argument.startswith("--"):
                ...
                yield CommandInputParameter(
                    ...
                    default=default,
                    inputBinding=dict(
                        position=position, itemSeparator=itemSeparator, prefix=prefix
                    ),
                )
                prefix = None
    ...
    def find_explicit_inputs(self):
        ...
        input_paths = [
            input.default.path for input in self.inputs if input.type in PATH_OBJECTS
        ]
