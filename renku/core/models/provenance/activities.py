...
class Activity(CommitMixin):
    ...
    generated = jsonld.container.list(
        Generation, context={"@reverse": "prov:activity"}, kw_only=True
    )
    ...
    def default_generated(self):
        ...
        for path, role in self.outputs.items():
            ...
            if str(path).startswith(os.path.join(client.renku_home, client.DATASETS)):
                ...
                entity = entity_cls(
                    commit=commit, client=client, path=str(path), parent=collection
                )

            if collection:
                collection.members.append(entity)

            results.append(Generation(activity=self, entity=entity, role=role))
        return results
...
@jsonld.s(
    ...
    context={"wfprov": "http://purl.org/wf4ever/wfprov#"},
    cmp=False,
)
class ProcessRun(Activity):
    ...
    qualified_usage = jsonld.ib(context="prov:qualifiedUsage", kw_only=True, type=Usage)
                ...
                if output_id not in existing_outputs:
                    self.outputs[
                        os.path.join(
                            next(
                                path
                                for path, usage in self.inputs.items()
...
class WorkflowRun(ProcessRun):
    ...
    _processes = jsonld.ib(
        context={"@reverse": "wfprov:wasPartOfWorkflowRun"},
    ...
    def default_children(self):
        ...
        def _load(step):
            ...
            if self.commit:
                ...
                data = (self.commit.tree / basedir / step.run).data_stream.read()
    ...
    def default_subprocesses(self):
        ...
        for step in reversed(self.process.topological_steps):
            ...
            for alias, source in step.in_.items():
                ...
                if source in ins:
                    ...
                    inputs[dependency.path] = attr.evolve(
                        dependency, role=alias, id=usage_id
                    )
                elif source in outs:
                    input_path = outs[source]
                    inputs[input_path] = Usage(
                        entity=entities[input_path], role=alias, id=usage_id
                    )
                else:
            ...
            subprocess.association = Association.from_activity(
                subprocess, commit=subprocess_entity_commit
            )
            ...
            for generation in subprocess.generated:
                ...
                if isinstance(entity, Collection):
                    entities.update(
                        **{member.path: member for member in entity.members}
                    )
        ...
        for output in self.process.outputs:
            ...
            for generated in self.subprocesses[step_id].generated:
                if generated.role == source:
                    results.append(
                        attr.evolve(generated, role=output.id, activity=self)
                    )
                    break
            else:
