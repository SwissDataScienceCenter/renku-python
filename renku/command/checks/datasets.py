...
def check_dataset_old_metadata_location(client, **kwargs):
    ...
    problems = (
        ...
        + "\n\t".join(click.style(str(path.relative_to(config.path)), fg="yellow") for path in old_metadata)
        + "\n"
    )
...
def check_missing_files(client, dataset_gateway: IDatasetGateway, **kwargs):
    ...
    for dataset in dataset_gateway.get_all_active_datasets():
        for file_ in dataset.files:
            path = config.path / file_.entity.path
    ...
    for dataset in dataset_gateway.get_provenance_tails():
        ...
        for file in dataset.files:
            if file.is_external:
                continue
            try:
                get_safe_relative_path(config.path / file.entity.path, config.path / data_dir)
            except ValueError:
