...
def _remove(sources, edit_command, client_dispatcher: IClientDispatcher, dataset_gateway: IDatasetGateway):
    ...
    def get_relative_path(path):
        ...
        abs_path = os.path.abspath(config.path / path)
        try:
            return str(Path(abs_path).relative_to(config.path))
        except ValueError:
        ...
        for dataset in all_datasets:
            ...
            if remove:
                ...
                for key in remove:
                    ...
                    delete_dataset_file(config.path / key, follow_symlinks=True)
    ...
    if client.check_external_storage():
        ...
        if existing:
            ...
            if communication.confirm('Do you want to edit ".gitattributes" now?'):
                edit_command(filename=str(config.path / ".gitattributes"))

    # Finally remove the files.
    files_to_remove = set(str(config.path / f) for f in files.values())
